package mx.charts.series
{
   ...
   public class LineSeries extends Series
   {
      ...
      private function initStyles() : Boolean
      {
         ...
         seriesStyle = HaloDefaults.createSelector("mx.charts.series.LineSeries",styleManager);
      ...
      override protected function commitProperties() : void
      {
         ...
         if(_loc1_)
         {
            if(!this._horizontalAxis)
            {
               if(dataTransform.axes[CartesianTransform.HORIZONTAL_AXIS] != _loc1_.horizontalAxis)
               {
            ...
            if(!this._verticalAxis)
            {
               if(dataTransform.axes[CartesianTransform.VERTICAL_AXIS] != _loc1_.verticalAxis)
               {
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         var _loc6_:int = int(_loc3_.filteredCache.length);
         var _loc8_:int = int(_loc3_.segments.length);
         if(_loc3_ == transitionRenderData && Boolean(_loc3_.elementBounds))
         {
         ...
         if(_loc5_ > 0)
         {
            ...
            if(_loc3_ == transitionRenderData && Boolean(_loc3_.elementBounds))
            {
               ...
               while(_loc7_ < _loc6_)
               {
                  ...
                  if(Boolean(_loc11_.itemRenderer) && (_loc11_.itemRenderer as Object).hasOwnProperty("invalidateDisplayList"))
                  {
            ...
            else
            {
               ...
               if(Boolean(chart) && Boolean(allSeriesTransform) && chart.chartState == 0)
               {
      ...
      override protected function updateData() : void
      {
         ...
         if(Boolean(dataTransform && dataTransform.getAxis(CartesianTransform.VERTICAL_AXIS) is NumericAxis) && Boolean(!(dataTransform.getAxis(CartesianTransform.VERTICAL_AXIS) is DateTimeAxis)) && (dataTransform.getAxis(CartesianTransform.VERTICAL_AXIS) as NumericAxis).direction == "inverted")
         {
            this._renderData.cache = this.reverseYValues(this._renderData.cache);
         }
         if(Boolean(dataTransform && dataTransform.getAxis(CartesianTransform.HORIZONTAL_AXIS) is NumericAxis) && Boolean(!(dataTransform.getAxis(CartesianTransform.HORIZONTAL_AXIS) is DateTimeAxis)) && (dataTransform.getAxis(CartesianTransform.HORIZONTAL_AXIS) as NumericAxis).direction == "inverted")
         {
      ...
      override protected function updateTransform() : void
      {
         ...
         if(Boolean(chart) && chart is CartesianChart)
         {
            _loc1_ = CartesianChart(chart);
            _loc2_ = int(_loc1_.series.length);
            _loc3_ = 0;
            while(_loc3_ < _loc2_)
            {
               if(_loc1_.mx_internal::getSeriesTransformState(_loc1_.series[_loc3_]))
               {
            ...
            {
               _loc1_.mx_internal::measureLabels();
            }
         }
      }
      ...
      override public function describeData(param1:String, param2:uint) : Array
      {
         ...
         if(param1 == CartesianTransform.VERTICAL_AXIS)
         {
            ...
            if((param2 & DataDescription.REQUIRED_BOUNDED_VALUES) != 0)
            {
               ...
               _loc7_ = getStyle("itemRenderer");
               if(_loc6_ > 0 && Boolean(_loc7_))
               {
                  _loc5_ = getStyle("stroke");
                  if(_loc5_)
                  {
                     _loc6_ += _loc5_.weight / 2;
                  }
         ...
         {
            if(param1 != CartesianTransform.HORIZONTAL_AXIS)
            {
               return [];
            }
            if(this._xField != "")
      ...
      override public function getAllDataPoints() : Array
      {
         ...
         var _loc1_:Array = [];
         if(Boolean(chart) && Boolean(chart.dataTipItemsSet) && Boolean(dataTipItems))
         {
            _loc1_ = dataTipItems;
         }
         else if(Boolean(chart) && Boolean(chart.showAllDataTips) && Boolean(this._renderData.filteredCache))
         {
         ...
         for(; _loc3_ < _loc2_; _loc3_++)
         {
            ...
            {
               _loc6_ = false;
               _loc7_ = int(this._renderData.filteredCache.length);
               _loc8_ = 0;
               while(_loc8_ < _loc7_)
               {
            ...
            if(_loc5_)
            {
               ...
               if(_loc10_ is SolidColorStroke)
               {
                  _loc9_.contextColor = SolidColorStroke(_loc10_).color;
               }
      ...
      override public function findDataPoints(param1:Number, param2:Number, param3:Number) : Array
      {
         ...
         var _loc8_:int = int(this._renderData.filteredCache.length);
         if(_loc8_ == 0)
         {
            return [];
         }
         if(this.sortOnXField == true)
         {
            ...
            while(true)
            {
               ...
               if(Boolean(dataTransform) && Boolean(dataTransform.getAxis(CartesianTransform.HORIZONTAL_AXIS) is NumericAxis) && (dataTransform.getAxis(CartesianTransform.HORIZONTAL_AXIS) as NumericAxis).direction == "inverted")
               {
                  _loc14_ = param1;
                  _loc15_ = _loc13_.x;
               }
         ...
         if(_loc6_)
         {
            ...
            if(_loc19_ is SolidColorStroke)
            {
               _loc18_.contextColor = SolidColorStroke(_loc19_).color;
            }
      ...
      override protected function getMissingInterpolationValues(param1:Object, param2:Array, param3:Object, param4:Array, param5:Number, param6:Object) : void
      {
         ...
         {
            _loc10_ = Number(param1[_loc9_]);
            _loc11_ = Number(param3[_loc9_]);
            _loc12_ = int(param5);
            if(isNaN(_loc10_))
            {
               if(_loc7_.length == 0)
               {
                  _loc10_ = _loc9_ == "x" ? Number(_loc8_[param5].x) : unscaledHeight;
               }
               else
               {
      ...
      {
         return dataTransform.invertTransform(param1.x - this.x,param1.y - this.y);
      }
      ...
      private function formatDataTip(param1:HitData) : String
      {
         ...
         _loc2_ += dataTransform.getAxis(CartesianTransform.HORIZONTAL_AXIS).formatForScreen(LineSeriesItem(param1.chartItem).xValue) + "\n";
         var _loc5_:String = dataTransform.getAxis(CartesianTransform.VERTICAL_AXIS).displayName;
         if(_loc5_ != "")
         {
            _loc2_ += "<i>" + _loc5_ + ":</i> ";
         }
         return _loc2_ + (dataTransform.getAxis(CartesianTransform.VERTICAL_AXIS).formatForScreen(LineSeriesItem(param1.chartItem).yValue) + "\n");
      }
...
class LineSeriesLegendMarker extends ProgrammaticSkin
{
   ...
   public function LineSeriesLegendMarker(param1:LineSeries)
   {
      super();
   ...
   override protected function updateDisplayList(param1:Number, param2:Number) : void
   {
      ...
      if(_loc3_ is SolidColorStroke)
      {
         _loc4_ = SolidColorStroke(_loc3_).color;
      }
      else if(_loc3_ is LinearGradientStroke)
      {
         ...
         {
            _loc4_ = Number(_loc6_.entries[0].color);
         }
      }
