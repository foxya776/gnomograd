   ...
   public class HScrollBar extends ScrollBarBase
   {
      ...
      override protected function updateSkinDisplayList() : void
      {
         ...
         if(getStyle("interactionMode") == InteractionMode.TOUCH && Boolean(viewport))
         {
            ...
            _loc2_ = 0;
            _loc3_ = Math.max(0,maximum - _loc10_);
         }
         else
         {
            _loc2_ = minimum;
         ...
         var _loc5_:Boolean = getStyle("fixedThumbSize") !== false;
         var _loc7_:Number = 0;
         var _loc8_:Number = 0;
         ...
         if(pendingValue < _loc2_)
         {
            if(!_loc5_)
            {
               _loc9_ = Math.max(Math.min(thumb.minWidth,_loc9_),_loc9_ + pendingValue);
            }
            _loc7_ = _loc2_;
         }
      ...
      override public function changeValueByPage(param1:Boolean = true) : void
      {
         ...
         {
            _loc2_ = pageSize;
            pageSize = Math.abs(viewport.getHorizontalScrollPositionDelta(param1 ? NavigationUnit.PAGE_RIGHT : NavigationUnit.PAGE_LEFT));
         }
      ...
      override protected function animatePaging(param1:Number, param2:Number) : void
      {
         ...
         if(viewport)
         {
            _loc3_ = Math.abs(viewport.getHorizontalScrollPositionDelta(param1 > value ? NavigationUnit.PAGE_RIGHT : NavigationUnit.PAGE_LEFT));
      ...
      override public function changeValueByStep(param1:Boolean = true) : void
      {
         ...
         {
            _loc2_ = stepSize;
            stepSize = Math.abs(viewport.getHorizontalScrollPositionDelta(param1 ? NavigationUnit.RIGHT : NavigationUnit.LEFT));
         }
