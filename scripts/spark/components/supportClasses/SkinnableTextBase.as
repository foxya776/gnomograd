package spark.components.supportClasses
{
   ...
   public class SkinnableTextBase extends SkinnableComponent implements IFocusManagerComponent, IIMESupport
   {
      ...
      override public function get maxWidth() : Number
      {
         ...
         return _loc2_ === undefined ? super.maxWidth : Number(_loc2_);
      }
      ...
      {
         if(Boolean(focusManager && focusManager.getFocus() != focusManager.findFocusManagerComponent(this)) && Boolean(this.prompt != null) && this.prompt != "")
         {
            if(this.text.length == 0)
            {
               if(Boolean(enabled) && Boolean(skin) && skin.hasState("normalWithPrompt"))
               {
                  return "normalWithPrompt";
               }
               if(Boolean(!enabled) && Boolean(skin) && skin.hasState("disabledWithPrompt"))
               {
      ...
      {
         return param1 == this.textDisplay || Boolean(super.isOurFocus(param1));
      }
      ...
      mx_internal function getHeightInLines() : Number
      {
         ...
         return _loc2_ === undefined ? NaN : Number(_loc2_);
      }
      ...
      private function textDisplayRemoved() : void
      {
         ...
         if(BitFlagUtil.isSet(uint(this.textDisplayProperties),TEXT_FLOW_PROPERTY_FLAG) || Boolean(BitFlagUtil.isSet(uint(this.textDisplayProperties),CONTENT_PROPERTY_FLAG)) && Boolean(_loc2_))
         {
      ...
      private function touchInteractionStartHandler(param1:TouchInteractionEvent) : void
      {
         ...
         if(Boolean(isIOS && _loc2_) && Boolean(_loc2_.mx_internal::isSoftKeyboardActive) && this.editable)
         {
            stage.focus = null;
         }
         this.clearMouseDownState();
