   ...
   public class DataGroup extends GroupBase implements IItemRendererOwner
   {
      ...
      private function removeAllItemRenderers() : void
      {
         ...
         if(Boolean(this.virtualRendererIndices) && this.virtualRendererIndices.length > 0)
         {
            ...
            _loc2_ = int(this.freeRenderers.length - 1);
            while(_loc2_ >= 0)
            {
               _loc3_ = this.freeRenderers.pop() as IVisualElement;
         ...
         {
            _loc1_ = int(this.indexToRenderer.length - 1);
            while(_loc1_ >= 0)
            {
               this.removeRendererAt(_loc1_);
               _loc1_--;
      ...
      private function createRendererForItem(param1:Object, param2:Boolean = true) : IVisualElement
      {
         ...
         {
            _loc4_ = this.itemRendererFunction(param1);
            if(_loc4_)
            {
               _loc3_ = _loc4_.newInstance();
            }
            else if(param1 is IVisualElement && param1 is DisplayObject)
            {
               _loc3_ = IVisualElement(param1);
            }
         }
         if(!_loc3_ && Boolean(this.itemRenderer))
         {
            _loc3_ = this.itemRenderer.newInstance();
         }
         if(!_loc3_ && param1 is IVisualElement && param1 is DisplayObject)
         {
            _loc3_ = IVisualElement(param1);
         }
         if(!_loc3_ && param2)
         {
            if(param1 is IVisualElement || param1 is DisplayObject)
            {
               _loc5_ = resourceManager.getString("components","cannotDisplayVisualElement");
            }
            else
            {
               _loc5_ = resourceManager.getString("components","unableToCreateRenderer",[param1]);
            }
      ...
      private function createItemRenderers() : void
      {
         ...
         _loc2_ = int(this.indexToRenderer.length);
         while(_loc2_ < _loc1_)
         {
      ...
      private function finishVirtualLayout() : void
      {
         ...
         var _loc1_:Boolean = this.isRecyclingOK();
         for each(_loc2_ in this.oldVirtualRendererIndices)
         {
            if(this.virtualRendererIndices.indexOf(_loc2_) != -1)
            {
               continue;
            }
            _loc4_ = this.indexToRenderer[_loc2_] as IVisualElement;
            delete this.indexToRenderer[_loc2_];
            _loc5_ = this.dataProvider.length > _loc2_ ? this.dataProvider.getItemAt(_loc2_) : null;
            if(_loc1_ && _loc5_ != _loc4_ && _loc4_ is IDataRenderer)
            {
               _loc4_.includeInLayout = false;
               _loc4_.visible = false;
               this.freeRenderers.push(_loc4_);
            }
            else if(_loc4_)
            {
               dispatchEvent(new RendererExistenceEvent(RendererExistenceEvent.RENDERER_REMOVE,false,false,_loc4_,_loc2_,_loc5_));
               super.removeChild(DisplayObject(_loc4_));
            }
         }
         _loc3_ = false;
         for each(_loc2_ in this.virtualRendererIndices)
         {
            _loc4_ = this.indexToRenderer[_loc2_] as IVisualElement;
            if(!_loc4_ || !_loc4_.visible || !_loc4_.includeInLayout)
            {
               continue;
            }
      ...
      {
         var _loc1_:int = int(this.freeRenderers.length);
         var _loc2_:int = 0;
         while(_loc2_ < _loc1_)
         {
            this.freeRenderers[_loc2_] = null;
            _loc2_++;
         }
      ...
      override public function getVirtualElementAt(param1:int, param2:Number = NaN, param3:Number = NaN) : IVisualElement
      {
         ...
         if(this.virtualLayoutUnderway)
         {
            ...
            if(_loc5_)
            {
               dispatchEvent(new RendererExistenceEvent(RendererExistenceEvent.RENDERER_ADD,false,false,_loc4_,param1,_loc6_));
            }
         }
         return _loc4_;
      }
      ...
      mx_internal function itemAdded(param1:Object, param2:int) : void
      {
         ...
         if(Boolean(layout) && layout.useVirtualLayout)
         {
            if(this.virtualRendererIndices)
            {
               _loc4_ = int(this.virtualRendererIndices.length);
               _loc5_ = 0;
               while(_loc5_ < _loc4_)
               {
      ...
      mx_internal function itemRemoved(param1:Object, param2:int) : void
      {
         ...
         {
            _loc5_ = -1;
            _loc6_ = int(this.virtualRendererIndices.length);
            _loc7_ = 0;
            while(_loc7_ < _loc6_)
            {
      ...
      private function addItemRendererToDisplayList(param1:DisplayObject, param2:int = -1) : void
      {
         ...
         var _loc4_:int = mx_internal::_overlay ? mx_internal::_overlay.numDisplayObjects : 0;
         var _loc5_:int = param2 != -1 ? param2 : int(super.numChildren - _loc4_);
         if(_loc3_ == this)
         {
