package spark.effects.animation
{
   ...
   public final class Animation
   {
      ...
      {
         var _loc2_:int = 0;
         if(Boolean(param1.motionPaths && param1.motionPaths.length > 0) && Boolean(param1.motionPaths[0]) && (param1.motionPaths[0].property == "width" || param1.motionPaths[0].property == "height"))
         {
      ...
      private static function removeAnimationAt(param1:int) : void
      {
         ...
         if(param1 >= 0 && param1 < activeAnimations.length)
         {
            ...
            _loc2_ = int(activeAnimations.length);
            _loc3_ = param1;
            while(_loc3_ < _loc2_)
            {
               _loc4_ = Animation(activeAnimations[_loc3_]);
               --_loc4_.id;
               _loc3_++;
            }
         }
      ...
      private static function timerHandler(param1:TimerEvent) : void
      {
         ...
         var _loc3_:int = int(activeAnimations.length);
         var _loc4_:int = 0;
         while(_loc4_ < activeAnimations.length)
         {
            _loc5_ = true;
            _loc6_ = Animation(activeAnimations[_loc4_]);
            if(_loc6_)
            {
         ...
         while(delayedStartAnims.length > 0)
         {
            ...
            _loc8_ = Number(delayedStartTimes[_loc7_]);
            if(_loc8_ >= Timeline.currentTime)
            {
               break;
            }
      ...
      {
         if(Boolean(timer) && Boolean(activeAnimations.length == 0) && delayedStartAnims.length == 0)
         {
            intervalTime = NaN;
            timer.reset();
            timer = null;
      ...
      private function removeFromDelayedAnimations() : void
      {
         ...
         {
            _loc1_ = int(delayedStartTimes[this]);
            _loc2_ = 0;
            while(_loc2_ < delayedStartAnims.length)
            {
               if(delayedStartAnims[_loc2_] == this)
               {
      ...
      {
         var _loc1_:int = 0;
         if(this.startDelay > 0 && delayedStartAnims.length > 0)
         {
            _loc1_ = 0;
            while(_loc1_ < delayedStartAnims.length)
            {
               if(this == delayedStartAnims[_loc1_])
               {
                  delete delayedStartTimes[this];
      ...
      private function addToDelayedAnimations(param1:Number) : void
      {
         ...
         {
            _loc5_ = int(delayedStartTimes[delayedStartAnims[_loc4_]]);
            if(_loc2_ < _loc5_)
            {
               _loc3_ = _loc4_;
               break;
      ...
      public function play() : void
      {
         var _loc1_:int = 0;
         var _loc2_:int = 0;
         var _loc3_:Vector.<Keyframe> = null;
      ...
      private function seek(param1:Number, param2:Boolean = false) : void
      {
         ...
         var _loc7_:int = 0;
         this.startTime = this.cycleStartTime = intervalTime - param1;
         this._doSeek = true;
         if(!this._isPlaying || this.playReversed)
         {
            ...
            if(param2 && this.startDelay > 0)
            {
               if(delayedStartTimes[this])
               {
                  ...
                  {
                     if(delayedStartAnims[_loc5_] == this)
                     {
      ...
      {
         var _loc1_:Number = Number(delayedStartTimes[this]);
         if(!isNaN(_loc1_))
         {
            this.delayTime = _loc1_ - Timeline.currentTime;
      ...
      private function start(param1:TimerEvent = null) : void
      {
         ...
         if(!this.playReversed)
         {
            ...
            {
               if(this == delayedStartAnims[_loc3_])
               {
                  _loc4_ = int(delayedStartTimes[this]);
                  _loc5_ = Timeline.currentTime - _loc4_;
                  if(_loc5_ > 0)
                  {
