package spark.effects.supportClasses
{
   ...
   public class AnimateInstance extends EffectInstance implements IAnimationTarget
   {
      ...
      override public function play() : void
      {
         ...
         var _loc7_:Vector.<Keyframe> = null;
         super.play();
         if(!this.motionPaths || this.motionPaths.length == 0)
         {
            _loc5_ = new Timer(duration,1);
            _loc5_.addEventListener(TimerEvent.TIMER,this.noopAnimationHandler);
            _loc5_.start();
            return;
         }
         ...
         {
            _loc6_ = MotionPath(this.motionPaths[_loc3_]);
            _loc7_ = _loc6_.keyframes;
            if(_loc7_)
            {
      ...
      private function finalizeValues() : Boolean
      {
         ...
         {
            _loc5_ = MotionPath(this.motionPaths[_loc4_]);
            _loc6_ = _loc5_.keyframes;
            if(!(!_loc6_ || _loc6_.length == 0))
            {
               if(!this.isValidValue(_loc6_[0].value))
               {
                  ...
                  else if((Boolean(mx_internal::playReversed || Effect(effect).mx_internal::transitionInterruption)) && Boolean(propertyChanges) && propertyChanges.start[_loc5_.property] !== undefined)
                  {
               ...
               while(_loc2_ < _loc6_.length)
               {
                  ...
                  if(!this.isValidValue(_loc7_.value))
                  {
                     ...
                     else if(Boolean(_loc2_ == _loc6_.length - 1) && Boolean(propertyChanges) && propertyChanges.end[_loc5_.property] !== undefined)
                     {
      ...
      private function addDisappearingTarget() : void
      {
         ...
         if(propertyChanges)
         {
            if("parent" in target && !target.parent)
            {
               ...
               if(Boolean(_loc1_) && Boolean(!_loc2_) && (_loc1_ is IVisualElementContainer || _loc1_ is SystemManager))
               {
      ...
      {
         var _loc3_:* = undefined;
         var _loc2_:Boolean = param1 in target;
         if(_loc2_)
         {
      ...
      private function setupParentLayout(param1:Boolean) : void
      {
         ...
         if(Boolean(_loc2_) && "autoLayout" in _loc2_)
         {
