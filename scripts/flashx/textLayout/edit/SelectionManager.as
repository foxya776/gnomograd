package flashx.textLayout.edit
{
   ...
   public class SelectionManager implements ISelectionManager
   {
      ...
      private static function computeSelectionIndexInContainer(param1:TextFlow, param2:ContainerController, param3:Number, param4:Number) : int
      {
         ...
         var _loc10_:Boolean = param1.computedFormat.direction == Direction.RTL;
         ...
         return _loc24_ != -1 ? _loc24_ : int(_loc6_ + _loc7_);
      }
      ...
      private static function computeSelectionIndexInLine(param1:TextFlow, param2:TextLine, param3:Number, param4:Number) : int
      {
         ...
         var _loc6_:Boolean = param1.computedFormat.blockProgression == BlockProgression.RL;
         ...
         if(_loc9_ == -1)
         {
            ...
            if(!_loc6_)
            {
               return _loc8_.x <= param2.x ? _loc5_.absoluteStart : int(_loc5_.absoluteStart + _loc5_.textLength - 1);
            }
            return _loc8_.y <= param2.y ? _loc5_.absoluteStart : int(_loc5_.absoluteStart + _loc5_.textLength - 1);
         }
      ...
      public function mouseDoubleClickHandler(param1:MouseEvent) : void
      {
         ...
         {
            _loc4_ = int(_loc2_.findNextWordBoundary(this.activeMark.position - _loc3_) + _loc3_);
         }
         else
         {
            _loc4_ = int(_loc2_.findPreviousWordBoundary(this.activeMark.position - _loc3_) + _loc3_);
         }
         ...
         else
         {
            ...
            else
            {
               if(this.anchorMark.position <= this.activeMark.position)
               {
                  _loc5_ = int(_loc6_.findPreviousWordBoundary(this.anchorMark.position - _loc7_) + _loc7_);
               }
               else
               {
                  _loc5_ = int(_loc6_.findNextWordBoundary(this.anchorMark.position - _loc7_) + _loc7_);
               }
      ...
      public function notifyInsertOrDelete(param1:int, param2:int) : void
      {
         ...
         while(_loc3_ < this.marks.length)
         {
            ...
            if(_loc4_.position >= param1)
            {
               if(param2 < 0)
               {
                  _loc4_.position = _loc4_.position + param2 < param1 ? param1 : int(_loc4_.position + param2);
               }
               else
               {
